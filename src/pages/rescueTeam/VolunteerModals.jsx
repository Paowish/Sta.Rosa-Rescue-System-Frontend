...
// Custom marker function based on severity
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white rounded-xl shadow-2xl w-[420px] max-w-[90vw] p-6 flex flex-col animate-in zoom-in-95 duration-200">
                ...
                <div className="flex gap-3">
                    ...
                    <button onClick={() => { if (onConfirm) onConfirm(); onClose(); }} className={`flex-1 py-2.5 rounded-lg text-sm font-medium text-white transition ${confirmColor}`}>{confirmText}</button>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white rounded-xl shadow-2xl w-[420px] max-w-[90vw] p-6 flex flex-col animate-in zoom-in-95 duration-200">
                ...
                <h3 className="text-xl font-bold text-gray-800 text-center mb-2">{title}</h3>
                <p className="text-gray-600 text-center text-sm mb-6">{message}</p>
                <button onClick={onClose} className={`py-2.5 rounded-lg text-sm font-medium text-white transition ${getButtonColor()}`}>OK</button>
            </div>
        </div>
    );
};
    ...
    useEffect(() => {
        const fetchIncidents = async () => {
            try {
                ...
                const response = await fetch(`${getApiUrl()}/incidents`, { headers: { 'Authorization': `Bearer ${token}` } });
                const data = await response.json();
                if (data.success) {
                    const activeIncidents = data.data.filter(inc => inc.status === 'Pending' || inc.status === 'Dispatched' || inc.status === 'Acknowledged');
                    setIncidents(activeIncidents);
                }
            } catch (error) { console.error('Error fetching incidents:', error); } finally { setFetching(false); }
        };
        fetchIncidents();
    }, []);
    ...
    const handleDispatch = async () => {
        ...
        try {
            ...
            const response = await fetch(`${getApiUrl()}/incidents/${selectedIncident}/assign`, {
                ...
                body: JSON.stringify({ responderIds: [volunteer.id], teamName: volunteer.name, dispatchNotes: `Dispatched to ${volunteer.name}` })
            });
            const data = await response.json();
            if (data.success) { alert(`✅ ${volunteer.name} dispatched!`); onDispatch(); onClose(); }
            else alert(`❌ Dispatch failed: ${data.message}`);
        } catch (error) { alert('❌ Failed to dispatch.'); } finally { setLoading(false); }
    };
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-white w-full max-w-4xl rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                <div className="bg-[#5e747f] px-6 py-4 flex justify-between items-center shrink-0">
                    ...
                    <button onClick={onClose} className="text-white hover:text-gray-200"><Icon icon="mdi:close" className="w-6 h-6" /></button>
                </div>
                ...
                <div className="flex flex-col md:flex-row h-full overflow-hidden">
                    ...
                    <div className="w-full md:w-1/3 border-r border-gray-200 bg-[#f5f6f8] overflow-y-auto max-h-[60vh] md:max-h-[70vh]">
                        ...
                        <div className="p-0 text-xs">
                            ...
                            <div className="bg-white divide-y divide-gray-100">
                                <div className="flex justify-between py-2.5 px-6"><span className="text-gray-500 font-medium">Volunteer ID</span><span className="text-gray-800 font-semibold">{volunteer.appId || 'N/A'}</span></div>
                                ...
                                <div className="flex justify-between py-2.5 px-6"><span className="text-gray-500 font-medium">Years Active</span><span className="text-gray-800 font-semibold">{getYearsActive()}</span></div>
                                <div className="flex justify-between py-2.5 px-6 bg-[#f7f8fa]"><span className="text-gray-500 font-medium">Current Assignment</span><span className="text-gray-800 font-semibold">{selectedIncident ? incidents.find(i => i._id === selectedIncident)?.incidentId || 'None' : 'None'}</span></div>
                            </div>
                            ...
                            <div className="bg-white p-5 flex flex-wrap gap-1.5">
                                {volunteer.details?.skills?.length > 0 ? volunteer.details.skills.slice(0, 8).map((s, idx) => (
                    ...
                    {/* Right Panel - Interactive Map & Incidents */}
