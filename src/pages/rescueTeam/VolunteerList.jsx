 ...
 * Displays a grid of accepted volunteers with real-time status
 */
    ...
    return (
        <div className="grid grid-cols-2 gap-5">
            {volunteers.length === 0 ? (
                ...
                volunteers.map((v) => {
                    // Get real-time live status from parent
                    ...
                    return (
                        ...
                        >
                            {/* Top status bar */}
                            <div className={`h-1.5 w-full ${liveStatus === 'On Scene' ? 'bg-orange-500' :
                                    liveStatus === 'En Route' ? 'bg-blue-500' :
                                        liveStatus === 'Dispatched' ? 'bg-red-500' :
                                            'bg-green-600'
                                }`}></div>
                                ...
                                <div className="border-t border-gray-200 pt-3 mt-2">
                                    ...
                                    <div className="flex flex-wrap">
                                        {v.details?.skills?.slice(0, 3).map((s, idx) => (
                                            <span
                                                key={idx}
                                                className="bg-blue-50 text-blue-600 text-[11px] font-medium px-2.5 py-1 rounded border border-blue-100 mr-1.5 mb-1.5"
                                            >
                                                {s}
                                            </span>
                                        ))}
 ...
 * Displays a list of pending applicants with actions
 */
                                    ...
                                    {app.tags && app.tags.length > 0 ? (
                                        <>
                                            {app.tags.map((t, idx) => (
                                                <span
                                                    key={idx}
                                                    className="bg-blue-50 border border-blue-200 text-blue-600 text-[9px] font-medium px-2 py-0.5 rounded-full truncate max-w-[100px]"
                                                >
                                                    {t}
                                                </span>
                                            ))}
                                ...
                                <div className="flex items-center flex-wrap gap-x-1.5 gap-y-1 text-[12px] text-gray-600 font-medium">
                                    ...
                                    <div className="flex items-center gap-[1px]">
                                        {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day) => (
                                            <div
                                                key={day}
                                                className={`w-4 h-2 rounded-sm ${app.availability && app.availability.includes(day) ? 'bg-[#15803d]' : 'bg-gray-200'
                                                    }`}
                                            ></div>
                                        ))}
                                    </div>
                                    <span className="ml-1 font-normal text-gray-500 text-[10px]">
                                        {app.availability && Array.isArray(app.availability) && app.availability.length > 0
                                            ? ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
                                                .filter(day => app.availability.includes(day))
                                                .join(' · ')
                                            : 'None selected'}
                                    </span>
                                </div>
