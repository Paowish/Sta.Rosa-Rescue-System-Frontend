                    ...
                    <div className={`rounded-lg shadow-2xl border-l-4 p-4 ${getNotificationColor(newNotification.type)} bg-white`}>
                        <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-semibold text-gray-800 break-words">{newNotification.title}</p>
                                <p className="text-xs text-gray-600 mt-1 break-words leading-relaxed">{newNotification.message}</p>
                                <p className="text-[10px] text-gray-400 mt-2">{formatTime(newNotification.createdAt)}</p>
                            </div>
                            <button onClick={dismissNotification} className="text-gray-400 hover:text-gray-600 transition-colors flex-shrink-0">
                ...
                <div className="bg-white rounded-lg shadow-lg border overflow-hidden">
                    ...
                    <div className="p-3 sm:p-4 border-b sticky top-0 bg-white">
                        <div className="flex justify-between items-center">
                            ...
                            {unreadCount > 0 && (
                                <button onClick={handleMarkAllAsRead} className="text-xs text-blue-500 hover:text-blue-700 transition-colors duration-200 hover:underline">
                                    Mark all as read
                                </button>
                            )}
                        </div>
                            ...
                            notifications.map((notif) => (
                                ...
                                >
                                    <div className="flex items-start gap-2 sm:gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="text-sm sm:text-base font-semibold text-gray-800 break-words">{notif.title}</p>
                                            <p className="text-xs text-gray-600 mt-1 break-words leading-relaxed">{notif.message}</p>
                                            <p className="text-[10px] sm:text-xs text-gray-400 mt-2">{formatTime(notif.createdAt)}</p>
                                        </div>
                                        {!notif.isRead && <div className="w-2 h-2 bg-blue-500 rounded-full mt-1 sm:mt-2 flex-shrink-0"></div>}
                                    </div>
                                </div>
                            ))
                        )}
